...
**`cmd/mockjev`.** A standalone fake TypeSafe server implementing the same wire format, used by the demo and e2e so they run without an API key:
...
| `scraper` | 3 IPs, 5 rps, sequential `/products/{n}`, fixed interval | `throttle` (≥) | `block` |
...
- [ ] Renamed to Portcullis, with backward-compatible env reading for one release.
- [ ] Go directive, toolchain, and dependencies per §3; `govulncheck` clean; CI green (lint, unit, integration, e2e, docker, bench-smoke).
- [ ] B1–B24 fixed, each with a regression test; conformance suite passes on both backends.
- [ ] Signals, detection, tiers, judges (rules, typesafe, mock), controller, guardrails G1–G10, audit, and admin APIs implemented to spec.
- [ ] Gateway mode works; the demo stack runs with `make demo` on a fresh clone with no API key.
- [ ] All e2e scenarios in §7.3 pass three times in a row (flake check).
- [ ] Performance budgets in §5.9 met and recorded.
- [ ] README, ARCHITECTURE, OPERATIONS, BENCHMARKS, and CHANGELOG (with Breaking section) written.
- [ ] §4 status column and this checklist updated to reflect reality.
